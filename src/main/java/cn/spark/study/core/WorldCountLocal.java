...
public class WorldCountLocal {
	...
	public static void main(String[] args) {
		...
		//在spark中，SparkContext是Spark所有功能的一个入口，你无论是用Java，Scala，python编写
		//都必须要有一个SparkContext，它的主要作用，包括初始化Spark应用程序所需的一些核心组件，包括
		//调度器（DAGSchedule、TaskScheduler），还会去到Spark Master节点上进行注册，等等
		//一句话，SparkContext，是Spark应用中，可以说是最重要的一个对象
		//但是呢，在Spark中，编写不同类型的Spark应用程序，使用的SparkContext是不同的，如果使用Scala，
		//使用的就是原生的SparkContext对象
		//但是如果使用Java开发，那么就是javaSparkContext对象
		//如果是SparkSQL程序，那么就是SQLContext、HiveContext
		//如果是开发SaprkStreaming程序，那么就是它独有的SparkContext
		//以此类推
		JavaSparkContext sc = new JavaSparkContext(conf);
		
		//第三步：要针对输入源（HDFS文件、本地文件，等等），创建一个初始化的RDD
		...
		//这就是Scala中的tuple，包含两个值
		//mapToPair这个算子，要求的是与PairFunction配合使用，第一个泛型参数代表了输入类型
		//第二个和第三个泛型参数，分别代表了tuple元素的第一个值和第二个值的类型
		...
		//最后返回的JavaPairRDD中的元素，也就是tuple，但是第一个值就是每个key，第二个值就是key的value
		...
		//一个Spark应用中，光是又transformation操作，是不行的，是不会执行的，必须要有一种叫做action
