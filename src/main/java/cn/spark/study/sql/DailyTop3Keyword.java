...
public class DailyTop3Keyword {
	...
	public static void main(String[] args) {
		...
		JavaRDD<String> rawRDD = sc.textFile("hdfs://hadoop-3:9000/spark-study/keyword.txt");
		...
		JavaRDD<String> filterRDD = rawRDD.filter(new Function<String, Boolean>() {
			...
			public Boolean call(String log) throws Exception {
				...
				List<String> citys = queryParamMap.get("city");
				if(citys.size() >0 && !citys.contains(city)){
					return false;
				}
				
				List<String> platforms = queryParamMap.get("plamform");
				if(platforms.size() > 0 && !platforms.contains(platform)){
					return false;
				}
				
				List<String> versions = queryParamMap.get("versions");
		...
		JavaPairRDD<String, String> top3DateKeywordUvRDD = dailyTop3KeywordRDD.mapToPair(
				...
				new PairFunction<Row, String, String>() {
					...
					public Tuple2<String, String> call(Row row) throws Exception {
						String date = String.valueOf(row.getString(0));
						String keyword = String.valueOf(row.getString(1));
						String uv = String.valueOf(row.getString(2));
